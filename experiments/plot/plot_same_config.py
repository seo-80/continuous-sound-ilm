...
for i in range(len(params_list)):
    ...
    for j in range(K):
        ...
        contourf = axs.contourf(
            x, y, z, levels=[level, 1], colors=[cluster_colors[j]], alpha=0.2
        )
    ...
    plt.savefig(
        os.path.join(OUTPUT_DIR, "last_generation", f"last_generation_{i}.png"),
        bbox_inches="tight",
    )
    plt.close(fig)
...
colors = [
    cluster_colors[len(cluster_colors) // 2],
    cluster_colors[len(cluster_colors) // 2 + 1],
]  # Use first colors from even/odd clusters
axs.bar(
    range(1, 3),
    [np.mean(distance_even), np.mean(distance_odd)],
    color=colors,
    alpha=0.8,
)
...
for i in range(2):
    ...
    axs.bar(
        ...
        hist,
        width=bar_width,
        color=cluster_colors[len(cluster_colors) // 2 + i],
        alpha=1.0,
        label=labels[i],
    )
...
for i in range(K):
    ...
    contourf = axs.contourf(
        x, y, z, levels=[level, 1], colors=[cluster_colors[i]], alpha=0.2
    )
for i in range(K):
    axs.arrow(
        config["m0"][i][0],
...
plt.savefig(
    os.path.join(
        OUTPUT_DIR, "final_generation_mean_with_arrow_and_distance_with_count.png"
    )
)
