...
parser.add_argument(
    "folder_name", nargs="?", type=str, default="all", help="input file path"
)
...
def generate_double_gradation(n):
    ...
    for i in range(n):
        if i % 2 == 0:
            ...
            color = tuple(
                start_color1[j] + (end_color1[j] - start_color1[j]) * t
                for j in range(3)
            )
        else:
            t = ((i - 1) // 2) / (n // 2)  # Normalized position
            color = tuple(
                start_color2[j] + (end_color2[j] - start_color2[j]) * t
                for j in range(3)
            )
        colors.append(color)
...
def generate_gradation(n, base_color):
    ...
    for i in range(n):
        t = i / (n - 1)
        color = tuple(
            start_color[j] + (end_color[j] - start_color[j]) * t for j in range(3)
        )
        colors.append(color)
...
for folder_name in folder_names:
    if os.path.exists(DATA_DIR + folder_name + "/config.json"):
        ...
        if os.path.exists(DATA_DIR + folder_name + "/excluded_data.nc"):
            try:
                excluded_data = xr.open_dataset(
                    DATA_DIR + folder_name + "/excluded_data.nc"
                )
            except:
                excluded_data = None

        params = np.load(
            DATA_DIR + folder_name + "/params.npy", allow_pickle=True
        ).item()
        retry_counts = np.load(DATA_DIR + folder_name + "/retry_counts.npy")
        metrics_data = xr.open_dataset(
            os.path.join(DATA_DIR, folder_name, "metrics.nc")
        )
        matched_data.append(
            {
                "X": X,
        ...
        if os.path.exists(DATA_DIR + folder_name + "/history.nc"):
            history_m = xr.open_dataset(
                DATA_DIR + folder_name + "/history.nc",
                drop_variables=list(params.keys() - {"m"}),
            )
            history_m_diff = np.array(
                [
                    history_m["m"][i] - history_m["m"][i - 1][-1]
                    for i in range(1, len(history_m["m"]))
                ]
            )
            ...
            last_generation_history = xr.open_dataset(
                DATA_DIR + folder_name + "/history.nc"
            ).sel(iter=iter - 1)
            ...
            def update(i):
                ...
                for k in range(K):
                    ...
                    matrix = (
                        last_generation_history["beta"][i][k].values
                    ...
                    axs.set_ylim(y_lim)
                    x, y = np.meshgrid(
                        np.linspace(*x_lim, 100), np.linspace(*y_lim, 100)
                    )
                    xy = np.column_stack([x.flat, y.flat])
                    z = multivariate_normal.pdf(xy, mean=mean, cov=covar).reshape(
                        x.shape
                    )
                    ...
                    levels = [level]
                    contour = axs.contour(
                        x, y, z, levels=levels, colors=[cluster_colors[k]]
                    )
                    artists.append(contour)
                    contourf = axs.contourf(
                        x,
                        y,
                        z,
                        levels=[level, 1],
            ...
            anim = animation.FuncAnimation(
                fig,
                update,
                frames=len(last_generation_history["m"]),
                interval=50,
                blit=True,
            )
            ...
            # Save animation
            anim.save(
                os.path.join(DATA_DIR, folder_name, "learning_animation.gif"),
                writer="pillow",
            )
            plt.close(fig)
        ...
        mean_step_diff = np.mean(
            np.linalg.norm(
                np.diff(params["m"][len(params["m"]) // 10 :], axis=0), axis=-1
            ),
            axis=0,
        )
        ...
        std_m = np.sqrt(
            np.var(params["m"][len(params["m"]) * 5 // 10 :], axis=0).mean(axis=1)
        )
        ...
        fig, axs = plt.subplots()
        expected_mahalanobis_mean = metrics_data["expected_mahalanobis"].mean(
            dim="simulation"
        )
        ...
        plt.savefig(
            os.path.join(DATA_DIR, folder_name, "expected_mahalanobis_mean.png")
        )
        ...
        for k in range(K):
            for i in range(1, iter):
                axs.plot(
                    params["m"][i - 1 : i + 1, k, 0],
                    params["m"][i - 1 : i + 1, k, 1],
                    color=colors[i],
                    alpha=0.5,
                )
            axs.scatter(
        ...
        distances = np.sqrt(
            np.sum(
                (params["m"] - np.mean(params["m"], axis=1, keepdims=True)) ** 2, axis=2
            )
        )
        for k in range(K):
            axs.plot(
                params["alpha"][:, k] / np.sum(params["alpha"], axis=1),
                label=f"Cluster {k+1}",
            )
        ...
        def update(i):
            ...
            if excluded_data is not None:
                if "iter" in excluded_data.dims:
                    ...
                    scatter = axs.scatter(
                        excluded[:, 0], excluded[:, 1], s=2, alpha=0.5, c="red"
                    )
                    artists.append(scatter)
        ...
        def update(i):
            ...
            for k in range(K):
                ...
                contour = axs.contour(
                    x, y, z, levels=[level], colors=[cluster_colors[k]]
                )
                artists.append(contour)
                contourf = axs.contourf(
                    x, y, z, levels=[level, 1], colors=[cluster_colors[k]], alpha=0.2
                )
                artists.append(contourf)
            ...
            for k in range(K):
                prior_mean = temp_config["m0"][k]
                marker = axs.scatter(
                    prior_mean[0], prior_mean[1], marker="x", color=cluster_colors[k]
                )
                artists.append(marker)
        ...
        ani = animation.FuncAnimation(fig, update, frames=iter, interval=500, blit=True)
        ani.save(
            DATA_DIR + folder_name + "/animation_colored_with_Z_arrow.gif",
            writer="pillow",
        )
        ...
        def update(i):
            ...
            for k in range(K):
                ...
                artists.append(contour)
                contourf = axs.contourf(
                    x, y, z, levels=[level, 1], colors=[cluster_colors[k]], alpha=0.15
                )
                artists.append(contourf)
        ...
        ani = animation.FuncAnimation(fig, update, frames=iter, interval=500, blit=True)
        ani.save(
            DATA_DIR + folder_name + "/animation_colored_with_Z.gif", writer="pillow"
        )
