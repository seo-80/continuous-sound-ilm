...
values_list = [[i for _ in range(4)] for i in base_values]
...
for value in values_list:
    ...
    for folder_name in folder_names:
        ...
        data_list.append(
            {"X": X, "Z": Z, "C": C, "params": params, "history_m": history_m}
        )
        exist = True
        break
    if not exist:
...
for data in data_list:
    ...
    convergence_times = [
        compute_convergence_time(
            history_m.isel(iter=i), data["params"]["m"][i - 1], 1 / 2
        )
        for i in range(1, history_m.dims["iter"] - 1)
    ]
