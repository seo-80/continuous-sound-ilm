    ...
    for iteration in range(max_iter):
        ...
        for k in range(K):
            responsibilities[:, k] = pi[k] * multivariate_normal.pdf(
                data, mean=mu[k], cov=beta[k] * np.linalg.inv(W[k])
            )
        ...
        for k in range(K):
            mu[k] = (
                beta_0 * mu_0
                + Nk[k] * (responsibilities[:, k][:, np.newaxis] * data).sum(axis=0)
            ) / (beta_0 + Nk[k])
            beta[k] = beta_0 + Nk[k]
            ...
            S = (
                (
                    ...
                    * np.einsum("ij,ik->ijk", x_centered, x_centered)
                ).sum(axis=0)
                + beta_0 * np.outer(mu_0, mu_0)
                + W_0
            )
            W[k] = W_0 + S
            nu[k] = nu_0 + Nk[k]
...
def iterated_leaning(simulation_times):
    ...
    data = np.vstack(
        [
            np.random.randn(data_sizes[0], 2) + np.array([3, 3]),
            np.random.randn(data_sizes[1], 2) + np.array([-3, -3]),
        ]
    )  # 100個の2次元データ
    K = 3  # クラスタ数
    ims = []
    ...
    for t in range(simulation_times):
        ...
        data = np.vstack(
            [
                rng.multivariate_normal(means[k], covariances[k], data_sizes[k])
                for k in range(K)
            ]
        )
        # ax.clear()
        ...
        im = ax.scatter(
            data[:, 0], data[:, 1], c=np.argmax(responsibilities, axis=1), cmap="Set1"
        )
        ims.append([im])
    print(type(fig))
