...
def e_step(data, weights, means, covariances):
    ...
    for k in range(num_components):
        ...
        responsibilities[:, k] = weights[k] * np.exp(-0.5 * term) / np.sqrt(np.linalg.det(covariances[k]) * (2 * np.pi)**d)
...
def m_step(data, responsibilities):
    ...
    for k in range(num_components):
        ...
        covariances[k] = np.einsum('i,ij,ik->jk', responsibilities[:, k], diff, diff) / n_k[k]
...
def log_likelihood(data, weights, means, covariances):
    ...
    for k in range(num_components):
        ...
        inv_cov = np.linalg.inv(covariances[k])
        term = np.einsum('ij,jk,ik->i', diff, inv_cov, diff)
        likelihood += np.sum(np.log(weights[k]) - 0.5 * (term + np.log(np.linalg.det(covariances[k])) + d * np.log(2 * np.pi)))
...
def em_algorithm(data, num_components, max_iter=100, tol=1e-4):
    ...
    for i in range(max_iter):
        ...
        if log_likelihood_prev is not None and abs(log_likelihood_curr - log_likelihood_prev) < tol:
            break
...
# データの生成
np.random.seed(0)
data = np.vstack([np.random.randn(100, 2) + np.array([3, 3]), np.random.randn(100, 2) + np.array([-3, -3])])
