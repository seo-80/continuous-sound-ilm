...
class MixtureDirichletGaussianWishartEvaluator:
    ...
    def expected_parameter_metrics(self, NIW_params, mixture_dirichlet_params):
        ...
        for m in range(n_dir_components):
            ...
            # m番目のディリクレ成分での分散
            var_weights_m = (alpha_m * (alpha_0_m - alpha_m)) / (
                alpha_0_m**2 * (alpha_0_m + 1)
            )
        ...
        for i, j in product(range(self.n_components), range(self.n_components)):
            if i < j:
                ...
                # print(params_i['nu_0'])
                exp_cov_sum = (
                    params_i["Psi_0"] / (params_i["nu_0"] - d - 1)
                ...
                base_var = (
                    1 / params_i["kappa_0"] + 1 / params_j["kappa_0"]
                ) * np.trace(inv(exp_cov_sum))
        ...
        for i, j in product(range(self.n_components), range(self.n_components)):
            if i < j:
                ...
                for m in range(n_dir_components):
                    ...
                    exp_cov_sum = params_i["Psi_0"] / (
                        params_i["nu_0"] - d - 1
                    ) + params_j["Psi_0"] / (params_j["nu_0"] - d - 1)

                    overlap = self._compute_overlap(
                        mean_diff, exp_cov_sum, weight_factor
                    )
        ...
        for m in range(n_dir_components):
            ...
            for k in range(self.n_components):
                ...
                complexity_scores.append(
                    dir_weights[m] * (weight_uncertainty + param_complexity)
                )
            ...
            for i, j in product(range(self.n_components), range(self.n_components)):
                if i < j:
                    weight_factor = np.sqrt(
                        (alpha_m[i] * alpha_m[j]) / (np.sum(alpha_m) ** 2)
                    )
        ...
        var_separation = (
            np.sum((separation_stds**2 + separation_means**2) * separation_weights)
            - mean_separation**2
        )
    ...
    def _compute_overlap(self, mean_diff, cov_sum, weight_factor):
        ...
        return (
            weight_factor * np.exp(exp_term) / np.sqrt((2 * np.pi) ** d * det(cov_sum))
        )

    def _compute_overlap_variance(
        self, mean_diff, cov_sum, params_i, params_j, dir_weights, dir_alphas, i, j
    ):
        """重なり係数の分散を計算"""
        d = mean_diff.shape[0]
        total_var = 0
        ...
        for m in range(len(dir_weights)):
            ...
            # ディリクレ成分での重みの不確実性
            weight_var = (
                alpha_m[i] * alpha_m[j] * (alpha_0_m - alpha_m[i] - alpha_m[j])
            ) / (alpha_0_m**3 * (alpha_0_m + 1))

            # パラメータの不確実性
            param_var = (1 / params_i["kappa_0"] + 1 / params_j["kappa_0"]) * np.trace(
                inv(cov_sum)
